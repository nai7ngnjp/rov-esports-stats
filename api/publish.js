...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1) อ่านไฟล์ปัจจุบัน (เอา sha + เนื้อหา)
    const cur = await gh(`${api}?ref=${encodeURIComponent(branch)}`);
    if (!cur.ok) { res.status(500).send("อ่าน index.html จาก GitHub ไม่ได้: " + (await cur.text())); return; }
    const curJson = await cur.json();
    const html = Buffer.from(curJson.content, "base64").toString("utf-8");
